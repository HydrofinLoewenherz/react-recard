      ...
      <Paper sx={{ my: 4 }}>
        <List>
          { cardLogs.map((log, i) =>
            <ListItem key={i}>
              <ListItemText>
                <Typography color={"lightblue"} component={"span"}>{ log.cardId }</Typography>
                <Typography component={"span"}> at </Typography>
                <Typography color={"lightblue"} component={"span"}>{ new Date(log.time).toDateString() }</Typography>
              </ListItemText>
              <ListItemIcon>
                { log.success ? <Done /> : <Clear /> }
              </ListItemIcon>
            </ListItem>
          )}
        </List>
